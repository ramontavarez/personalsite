...
const Game = ({ cards }: { cards: CardDataInterface[] }) => {
    ...
    return (
        <div className="px-4 container max-w-screen-lg lg:px-24 mb-8">
            <div className="w-full">
                <div className="w-full flex flex-wrap gap-1 mb-4 items-center">
                    <h3 className='font-semibold mr-2'>O que eu mais utilizo:</h3>
                    {matches.length > 0 ? matches.map((match, index) => (
                        <Badge key={index}>{match}</Badge>
                    )) : (
                        <span className="text-sm">Clique nos cards abaixo para descobrir 😃</span>
                    )}
                </div>
            </div>
            <div className='w-full rounded-sm border-2 p-0 lg:p-2 grid grid-cols-3 lg:grid-cols-6 gap-2'>
